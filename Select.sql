...
#Q1. What is the earliest formed band in the data set?
select * from metal_bands order by formed;
#Q2. How many rows are duplicates?
select distinct * from metal_bands;
#Q3.How many bands have white or black in name?
select distinct * from metal_bands where (band_name ilike '%white%') or 
(band_name ilike'%black%');
select DISTINCT * from metal_bands where band_name ilike '%white%' or 
band_name ilike'%black%';
#Q4.
select DISTINCT *from metal_bands where (origin ='Germany') 
and (style ilike'%Viking folk%') order by fans desc 
#Q5.
select*from metal_bands where split is null order by formed
